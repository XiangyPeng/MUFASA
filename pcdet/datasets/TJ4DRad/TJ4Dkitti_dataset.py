...
class TJ4DRadKittiDataset(DatasetTemplate):
    ...
    def get_image(self, idx):
        ...
        # image /= 255.0
        # return image
        return np.zeros([1216,1936,3], dtype=np.float32)
    # 没有图像
    def get_image_shape(self, idx):
        return np.array([1216,1936], dtype=np.int32)
        # img_file = self.root_split_path / 'image_2' / ('%s.jpg' % idx)
        # assert img_file.exists()
    ...
    def evaluation(self, det_annos, class_names, **kwargs):
        ...
        from .TJ4Deval import TJ4Deval  as TJ4Deval_eval 
        ...
        ap_result_str2, ap_dict2 =  TJ4Deval_eval.kitti_eval(eval_gt_annos2, eval_det_annos2, class_names)

        
        return ap_result_str, ap_dict,ap_result_str2
...
def create_kitti_infos(dataset_cfg, class_names, data_path, save_path, workers=4):
    dataset = TJ4DRadKittiDataset(dataset_cfg=dataset_cfg, class_names=class_names, root_path=data_path, training=False)
...
def maintest(args):
    ...
    if args.func == 'create_TJ4DRadSet_infos':  
        import yaml
        from pathlib import Path
        ...
        once_data_path = ROOT_DIR / 'data' / 'TJ4DRadSet_4DRadar'
        once_save_path = ROOT_DIR / 'data' / 'TJ4DRadSet_4DRadar'
        # 这个没有
        if args.runs_on == 'cloud':
            once_data_path = Path('/cache/TJ4DRadSet_4DRadar/')
            once_save_path = Path('/cache/TJ4DRadSet_4DRadar/')
            dataset_cfg.DATA_PATH = dataset_cfg.CLOUD_DATA_PATH

        create_kitti_infos(
            dataset_cfg=dataset_cfg,
            class_names=['Car', 'Pedestrian', 'Cyclist'],
            data_path=ROOT_DIR / 'data' / 'TJ4DRadSet_4DRadar',
            save_path=ROOT_DIR / 'data' / 'TJ4DRadSet_4DRadar'
        )

if __name__ == '__main__':
    import sys
    if sys.argv.__len__() > 1 and sys.argv[1] == 'create_TJ4DRadSet_infos':
        import yaml
        from pathlib import Path
        from easydict import EasyDict
