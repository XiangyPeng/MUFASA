...
class QueryAndGroup_pattern(nn.Module):
    ...
    def forward(self, xyz: torch.Tensor, xyz_batch_cnt: torch.Tensor,
        ...
        covariance_matrix = torch.bmm(grouped_xyz, grouped_xyz.permute(0, 2, 1))  # (batch_size, 3, 3)
        # B*N,3
        # value,sourt_index = torch.sort( torch.linalg.eigvalsh(covariance_matrix), dim=-1, descending=True)  # 
        # value, _ = torch.symeig(covariance_matrix, eigenvectors=False)

        # value =torch.linalg.eigvalsh(covariance_matrix)
        value,_ = torch.linalg.eig(covariance_matrix)
