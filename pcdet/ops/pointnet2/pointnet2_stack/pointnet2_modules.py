...
def build_local_aggregation_module_pattern(input_channels, config):
    ...
    if local_aggregation_name == 'StackSAModuleMSG':
        ...
        cur_layer = StackSAModuleMSG_pattern(
...
class StackSAModuleMSG_pattern(nn.Module):
    ...
    def __init__(self, *, radii: List[float], nsamples: List[int], mlps: List[List[int]],
        ...
        self.mlps_pattern = nn.ModuleList()
        
        self.mlps_pattern_fusion = nn.ModuleList()
        for i in range(len(radii)):
            radius = radii[i]
            nsample = nsamples[i]
            # self.groupers.append(pointnet2_utils.QueryAndGroup(radius, nsample, use_xyz=use_xyz))
            self.groupers.append(pointnet2_utils.QueryAndGroup_pattern(radius, nsample, use_xyz=use_xyz))
            mlp_spec = mlps[i]
            if use_xyz:
                mlp_spec[0] += 3
            ...
            shared_mlps_pattern.extend([
                nn.Linear(3, 32),
                # nn.BatchNorm2d(mlp_spec[k + 1]),
                nn.ReLU()
            ])
            self.mlps_pattern.append(nn.Sequential(*shared_mlps_pattern))
            
            
            shared_mlps_pattern_fusion = []
         
            shared_mlps_pattern_fusion.extend([
                nn.Linear(mlp_spec[k + 1]+32, mlp_spec[k + 1]),
                # nn.BatchNorm2d(mlp_spec[k + 1]),
                nn.ReLU()
            ])
            self.mlps_pattern_fusion.append(nn.Sequential(*shared_mlps_pattern_fusion))
    ...
    def forward(self, xyz, xyz_batch_cnt, new_xyz, new_xyz_batch_cnt, features=None, empty_voxel_set_zeros=True):
        ...
        new_features_list = []
        for k in range(len(self.groupers)):
            new_features, ball_idxs,value_pattern = self.groupers[k](
                xyz, xyz_batch_cnt, new_xyz, new_xyz_batch_cnt, features
            )  # (M1 + M2, C, nsample)
            new_features = new_features.permute(1, 0, 2).unsqueeze(dim=0)  # (1, C, M1 + M2 ..., nsample)
            # 
            ...
            new_features = self.mlps_pattern_fusion[k](torch.cat((new_features,point_pattern_feature),dim=-1))
